...
function sort(studentArray, houseNames, sortOrders, randomSeed, existingSort) {
  ...
  let numHouses = houseNames.length;
  let houses = Array(numHouses);
  let categories = {};
  let table = new Table(sortOrders);
  ...
  houseNames.sort(function(a, b) {
    return existingSort[a].length - existingSort[b].length;
  });
